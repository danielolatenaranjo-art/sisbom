...
class TelViewModel(application: Application) : AndroidViewModel(application) {
    ...
    var personnelList = mutableStateListOf<PersonItem>()
    var activeCentralSession by mutableStateOf<CentralSession?>(null)
    var isStartingCentralSession by mutableStateOf(false)
    var centralSessionError by mutableStateOf("")
    ...
    fun subscribeToData() {
        ...
        viewModelScope.launch {
            repository.getActiveCentralSessionFlow()
                .catch { it.printStackTrace() }
                .collectLatest { session ->
                    activeCentralSession = session
                }
        }
    ...
    fun loginCentralWithCredentials(userStr: String, passStr: String, onResult: (Boolean, String) -> Unit) {
        val u = userStr.trim()
        val p = passStr.trim()
        if (u.isBlank() || p.isBlank()) {
            centralSessionError = "Complete todos los campos"
            onResult(false, centralSessionError)
            return
        }

        isStartingCentralSession = true
        centralSessionError = ""

        viewModelScope.launch {
            // 1. Buscar en personnelList
            var matchedUser = personnelList.find {
                it.idRegistro.equals(u, ignoreCase = true) && it.contrasena == p
            }

            // 2. Fallback de búsqueda directa en Firestore
            if (matchedUser == null) {
                matchedUser = repository.findPersonByCredentials(u, p)
            }

            if (matchedUser == null) {
                isStartingCentralSession = false
                centralSessionError = "Credenciales incorrectas"
                onResult(false, centralSessionError)
                return@launch
            }

            val isActivo = matchedUser.activo.equals("SI", ignoreCase = true) ||
                    matchedUser.activo.equals("1", ignoreCase = true) ||
                    matchedUser.activo.equals("S", ignoreCase = true) ||
                    matchedUser.activo.isBlank()

            if (!isActivo) {
                isStartingCentralSession = false
                centralSessionError = "Usuario Inactivo"
                onResult(false, centralSessionError)
                return@launch
            }

            repository.startCentralSession(
                user = matchedUser,
                onSuccess = {
                    isStartingCentralSession = false
                    onResult(true, "Sesión de Central iniciada con éxito")
                },
                onFailure = { err ->
                    isStartingCentralSession = false
                    centralSessionError = err.message ?: "Error al iniciar sesión en Central"
                    onResult(false, centralSessionError)
                }
            )
        }
    ...
    fun startCentralOperatorSession(user: PersonItem, onResult: (Boolean, String) -> Unit) {
        isStartingCentralSession = true
        centralSessionError = ""
        viewModelScope.launch {
            repository.startCentralSession(
    ...
    fun closeCentralOperatorSession(onResult: (Boolean, String) -> Unit) {
        val currentSessionId = activeCentralSession?.idInicio ?: ""
        viewModelScope.launch {
            repository.closeCentralSession(
                sessionId = currentSessionId,
                onSuccess = {
                    onResult(true, "Sesión de Central finalizada")
                },
                onFailure = { err ->
                    onResult(false, err.message ?: "Error al cerrar sesión")
                }
            )
        }
    }
