...
def compile_tailwind():
    ...
    os.makedirs('comandancia/libs', exist_ok=True)
    ...
    try:
        ...
        print("Compiling comandancia/libs/tailwind.css...")
        subprocess.run([tailwind_bin, '-i', 'tailwind-input.css', '-o', 'comandancia/libs/tailwind.css', '--minify'], check=True)
...
def build():
    ...
    # 0.5 Generate embedded client logos (Placilla, etc.)
    print("Generating embedded client logos...")
    try:
        import generate_embedded_logos
        generate_embedded_logos.generate()
    except Exception as e:
        print(f"Warning: Could not generate embedded logos: {e}")

    # 1. Download/setup resources for both modules
    print("Setting up Comandancia resources...")
    download_external_resources('comandancia')
    patch_firebase_files('comandancia')
