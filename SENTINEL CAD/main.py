...
# Import bundled assets. If not generated, fallback to serving directly from disk
try:
    from bundled_assets import ASSETS
except ImportError:
    ASSETS = {}
    print("Modo desarrollo: sirviendo assets directamente desde el disco.")
...
class InMemoryHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Intercept SENTINEL brand logo requests (from LOGOS DE SENTINEL)
        for s_logo in ['sentinel_cad_logo.png', 'sentinel_icon.png', 'sentinel_vertical.png', 'logo 2.png', 'logo 1.png']:
            if path.endswith(s_logo):
                s_path = os.path.join(get_exe_dir(), s_logo)
                if not os.path.isfile(s_path):
                    s_path = os.path.join(get_exe_dir(), 'central', s_logo)
...
class Api:
    ...
    def save_list_pdf(self, doc_id, cycle_year, sequence_num, print_html_content):
        try:
            ...
            print(f"SENTINEL PDF: Generando borrador temporal HTML en {temp_html_path}", flush=True)
                    ...
                    def poll_task_completion():
                        try:
                            while not task.IsCompleted:
                                time.sleep(0.1)
                            print(f"SENTINEL PDF: PDF generado exitosamente en {dest_pdf_path}", flush=True)
                        except Exception as e:
                            print(f"SENTINEL PDF ERROR: Error en hilo de impresión: {e}", flush=True)
                        finally:
                            ...
                            try:
                                ...
                                print(f"SENTINEL PDF ERROR: No se pudo eliminar archivo temporal: {ex}", flush=True)
                                
                    t = threading.Thread(target=poll_task_completion)
                    t.daemon = True
                    t.start()
                except Exception as err:
                    print(f"SENTINEL PDF ERROR: Error en callback de carga: {err}", flush=True)
                    print_window.destroy()
                    if os.path.exists(temp_html_path):
                        os.remove(temp_html_path)
            ...
            print(f"SENTINEL PDF ERROR: No se pudo iniciar la generación de PDF: {e}", flush=True)
            return False
    ...
    def check_should_backup(self):
        try:
            ...
            print(f"SENTINEL BACKUP ERROR: Error al comprobar fecha: {e}", flush=True)
            return True
    ...
    def save_database_backup(self, backup_data_json):
        try:
            ...
            print(f"SENTINEL BACKUP: Copia de seguridad guardada con éxito en {backup_path}", flush=True)
            return True
        except Exception as e:
            print(f"SENTINEL BACKUP ERROR: Error al guardar copia de seguridad: {e}", flush=True)
    def load_local_db(self, filename):
        try:
            ...
            print(f"SENTINEL DB ERROR: Error al cargar {filename} desde disco: {e}", flush=True)
            return None
    ...
    def save_local_db(self, filename, content_str):
        try:
            ...
            print(f"SENTINEL DB ERROR: Error al guardar {filename} en disco: {e}", flush=True)
            return False
    ...
    def send_to_whatsapp(self, text):
        try:
            ...
            try:
                ...
                print(f"SENTINEL WHATSAPP CLIPBOARD ERROR: {ep}", flush=True)
            ...
            try:
                ...
                print("SENTINEL WHATSAPP: Aplicación de escritorio WhatsApp.exe iniciada con éxito.", flush=True)
                return True
            except Exception as ew:
                print(f"SENTINEL WHATSAPP DESKTOP FALLBACK: No se pudo abrir protocolo whatsapp:// ({ew}). Abriendo navegador...", flush=True)
                webbrowser.open(f"https://web.whatsapp.com/send?text={encoded_text}")
                return True
        except Exception as e:
            print(f"SENTINEL WHATSAPP GENERAL ERROR: {e}", flush=True)
            return False

    def trigger_ota_update(self, latest_version, download_url):
        print(f"SENTINEL OTA: Iniciando descarga e instalación de actualización V.{latest_version}...", flush=True)
        try:
            exe_dir = get_exe_dir()
            ...
            def download_and_restart():
                try:
                    ...
                    import subprocess
                    print(f"SENTINEL OTA: Descargando desde {download_url}...", flush=True)
                    urllib.request.urlretrieve(download_url, new_exe_path)
                    print(f"SENTINEL OTA: Descarga completada en {new_exe_path}.", flush=True)
...
exit
""")
                    print(f"SENTINEL OTA: Script de actualización bat creado en {bat_path}.", flush=True)
                    ...
                    print(f"SENTINEL OTA: Reiniciando proceso...", flush=True)
                    os._exit(0)
                except Exception as ex:
                    print(f"SENTINEL OTA ERROR: Falla durante la descarga/instalación: {ex}", flush=True)
            ...
            print(f"SENTINEL OTA ERROR: No se pudo iniciar el proceso de actualización: {e}", flush=True)
            return False
    ...
    def check_license(self, license_key=None):
        ...
        try:
                ...
                try:
                    ...
                    with urllib.request.urlopen(req, timeout=6) as response:
                        ...
                        if res_data.get("nombreMostrar") and self._window:
                            try:
                                ...
                                self._window.set_title(f"{res_data['nombreMostrar']} - SENTINEL CAD ({assigned_mod_label})")
                            except Exception as e:
                    ...
                    try:
                        ...
                        if err_body.get("nombreMostrar") and self._window:
                            try:
                                self._window.set_title(f"{err_body['nombreMostrar']} - SENTINEL CAD (Inactivo)")
                            except Exception as ex:
                    ...
                    with open(cache_path, "r", encoding="utf-8") as cache_file:
                        ...
                        if cached_res.get("nombreMostrar") and self._window:
                            try:
                                ...
                                self._window.set_title(f"{cached_res['nombreMostrar']} - SENTINEL CAD ({assigned_mod_label}) (Offline)")
                            except Exception as ex:
            ...
            return {
                "authorized": False,
                "reason": f"No se pudo conectar al Servidor Central SENTINEL. Detalle: {last_err}"
            }
        except Exception as e:
    ...
    def reverse_geocode(self, lat, lng):
        ...
        try:
            ...
            req = urllib.request.Request(
                url,
                headers={
                    'User-Agent': 'SENTINEL-CAD/2.1.1 (contacto@sentinel.cl)'
                }
            )
        ...
        try:
            ...
            req2 = urllib.request.Request(url2, headers={'User-Agent': 'SENTINEL-CAD/2.1.1'})
                ...
                for _ in range(15):
                    try:
                        ...
                        if res:
                            ...
                            print(f"SENTINEL OTA: Notificación enviada al frontend (V.{latest_version})", flush=True)
                            break
                    except Exception as ej:
                        print(f"SENTINEL OTA: Esperando carga para enviar actualización: {ej}", flush=True)
                    time.sleep(2)
        except Exception as e:
        ...
        try:
            with open(cache_path, "r", encoding="utf-8") as cache_file:
                ...
                if cached_res.get("nombreMostrar"):
                    window_title = f"{cached_res['nombreMostrar']} - SENTINEL CAD (Central de Alarmas)"
        except Exception:
            pass
